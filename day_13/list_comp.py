...
one_dime = [num for list_of_nums in list_of_lists for nums in list_of_nums for num in nums]
print(f'2. {one_dime}')
...
new_list = [[tup[0].upper(), tup[0][:3].upper(), tup[1].upper()]
            for lst in countries for tup in lst]
print(f'4. {new_list}')
...
lst_of_dicts = [{'country': country.upper(), 'city': city.upper()}
...
names = [[('Adeleke', 'Olasope')], [('David', 'Smith')],
         [('Donald', 'Trump')], [('Bill', 'Gates')]]
lst_of_strings = [fname + ' ' + lname for [(fname, lname)] in names]
print(f'6. {lst_of_strings}')


# # # 28.	Write a lambda function which can solve a slope or y-intercept of linear functions.

intercept = lambda y2, y1, x2, x1: (y2 - y1) / (x2 - x1)
print(f'{intercept(8,3,8,2):2f}')
